...
public final class ObjectHandler {
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				// artisian workshop stairs
					...
					player.setNextForceMovement(new NewForceMovement(player, 1, dest, 2, Utils.getFaceDirection(dest.getX() - player.getX(), dest.getY() - player.getY())));
				...
				// start of varrock dungeon
					...
					Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2207, 5346, 0));
				}
				// crucible
				else if (id == 67050)
	...
	private static boolean handleGate(Player player, WorldObject object) {
		...
		if (object.getRotation() == 0) {
			...
			if (World.removeObjectTemporary(object, 60000) && World.removeObjectTemporary(otherDoor, 60000)) {
	...
	public static void handleItemOnObject(final Player player, final WorldObject object, final int interfaceId, final Item item) {
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				else if ((item.getId() >= 1704 && item.getId() <= 1710 && item.getId() % 2 == 0) || (item.getId() >= 10356 && item.getId() <= 10366 && item.getId() % 2 == 0) || (item.getId() == 2572 || (item.getId() >= 20653 && item.getId() <= 20657 && item.getId() % 2 != 0))) {
					for (Item item : player.getInventory().getItems().getItems()) {
						if (item == null)
							continue;
