...
public class PlayerCombat extends Action {
	...
	private static final int FIRE_SPELL = 3;
	...
	public int mageAttack(final Player player, int spellId, boolean autocast) {
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 25: // air strike
				...
				World.sendProjectile(player, target, 91, 18, 18, 50, 50, 0, 0);
				return 5;
			case 28: // water strike
				...
				delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, baseDamage)));
				World.sendProjectile(player, target, 94, 18, 18, 50, 50, 0, 0);
				return 5;
			case 36:// bind
	...
	public int getMagicMaxHit(Player player, int baseDamage) {
		...
		if (target instanceof Player) { // old player magic formula
			...
			def = p2.getSkills().getLevel(Skills.DEFENCE) + (p2.getCombatDefinitions().getBonuses()[CombatDefinitions.MAGIC_DEF] * 2.5);
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe") || weaponName.contains("knife") || weaponName.contains("dart") || weaponName.contains("javelin") || weaponName.contains("thrownaxe")) {
					...
					if (player.getEquipment().getWeaponId() == 20171) {
						player.getCharges().addCharges(20173, ItemConstants.getItemDefaultCharges(20173), Equipment.SLOT_WEAPON);
	...
	public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double specMultiplier, boolean usingSpec) {
		...
		if (defenceAffects) {
			...
			if (!Combat.rollHit(att, def))
				return 0;
			/*
			 * double att = player.getSkills().getLevel(ranging ? 4 : 0) + player.getCombatDefinitions().getBonuses()[ranging ? 4 :
			 * CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)]; if (weaponId == -2) { att += 82; } att *= ranging ?
			 * player.getPrayer().getRangeMultiplier() : player .getPrayer().getAttackMultiplier(); if (fullVoidEquipped(player, ranging
			 * ? (new int[] { 11664, 11675 }) : (new int[] { 11665, 11676 }))) att *= 1.1; if (ranging) att *=
			 * player.getAuraManager().getRangeAccurayMultiplier(); double def = 0; if (target instanceof Player) { Player p2 = (Player)
			 * target; def = (double) p2.getSkills().getLevel(Skills.DEFENCE) + (2 * p2.getCombatDefinitions().getBonuses()[ranging ? 9 :
			 * CombatDefinitions .getMeleeDefenceBonus(CombatDefinitions .getMeleeBonusStyle(weaponId, attackStyle))]);
			 * 
			 * def *= p2.getPrayer().getDefenceMultiplier(); if (!ranging) { if (p2.getFamiliar() instanceof Steeltitan) def *= 1.15; } }
			 * else { NPC n = (NPC) target; def = n.getBonuses() != null ? n.getBonuses()[ranging ? 9 : CombatDefinitions
			 * .getMeleeDefenceBonus(CombatDefinitions .getMeleeBonusStyle(weaponId, attackStyle))] : 0; def *= 2; if(n.getId() == 1160
			 * && fullVeracsEquipped(player)) def *= 0.6; } if (usingSpec) { double multiplier = 0.25 + specMultiplier;///* 0.25 +
			 ...
			 * 
			 * if (prob > 0.90) // max, 90% prob hit so even lvl 138 can miss at // lvl 3 prob = 0.90; else if (prob < 0.05) // minimun
			 * 5% so even lvl 3 can hit lvl 138 prob = 0.05; if (prob < Math.random()) return 0;
			 */
		}
	...
	private final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double specMultiplier) {
		if (!ranging) {
			...
			/*
			 * //whip hiting 450 no pot no pray? lmao nty int strLvl = player.getSkills().getLevel(Skills.STRENGTH); int strBonus =
			 * player .getCombatDefinitions().getBonuses()[CombatDefinitions.STRENGTH_BONUS ]; double strMult =
			 * player.getPrayer().getStrengthMultiplier(); double xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle); double
			 * style = xpStyle == Skills.STRENGTH ? 3 : xpStyle == CombatDefinitions.SHARED ? 1 : 0; int dhp = 0; double dharokMod = 1.0;
			 * double hitMultiplier = 1.0; if (fullDharokEquipped(player)) { dhp = player.getMaxHitpoints() - player.getHitpoints();
			 * dharokMod = (dhp * 0.001) + 1; } if (fullVoidEquipped(player, 11665, 11676)) { hitMultiplier *= 1.1; } hitMultiplier *=
			 * specMultiplier; double cumulativeStr = (strLvl * strMult + style) * dharokMod; return (int) ((14 + cumulativeStr +
	...
	/*
	 * public int getRandomMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean defenceAffects, double
	 * specMultiplier, boolean usingSpec) { Random r = new Random(); max_hit = getMaxHit(player, weaponId, attackStyle, ranging,
	 * usingSpec, specMultiplier); double accuracyMultiplier = 1.0; if (defenceAffects) { accuracyMultiplier =
	 * getSpecialAccuracyModifier(player); } if (ranging && defenceAffects) { double accuracy = accuracyMultiplier *
	 * getRangeAccuracy(player, attackStyle) + 1; double defence = getRangeDefence(target) + 1; if (r.nextInt((int) accuracy) <
	 * r.nextInt((int) defence)) { return 0; } } else if (defenceAffects) { double accuracy = accuracyMultiplier *
	 * getMeleeAccuracy(player, attackStyle, weaponId) + 1; double defence = getMeleeDefence(target, attackStyle, weaponId) + 1;
	 * if (r.nextInt((int) accuracy) < r.nextInt((int) defence)) { return 0; } } int hit = r.nextInt(max_hit + 1); if (target
	 * instanceof NPC) { NPC n = (NPC) target; if (n.getId() == 9463 && hasFireCape(player)) hit += 40; } if
	 * (player.getAuraManager().usingEquilibrium()) { int perc25MaxHit = (int) (max_hit * 0.25); hit -= perc25MaxHit; max_hit -=
	 * perc25MaxHit; if (hit < 0) hit = 0; if (hit < perc25MaxHit) hit += perc25MaxHit;
	 * 
	 * } return hit; }
	 * 
	 ...
	 */
	/*
	 * public static double getMeleeAccuracy(Player e, int attackStyle, int weaponId) { int style = attackStyle == 0 ? 3 :
	 * attackStyle == 2 ? 1 : 0; int attLvl = e.getSkills().getLevel(Skills.ATTACK); int attBonus = e.getCombatDefinitions
	 * ().getBonuses()[CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle)]; if (weaponId == -2) { attBonus += 82; }
	 * double attMult = 1.0 * e.getPrayer().getAttackMultiplier(); double accuracyMultiplier = 1.0; if (fullVoidEquipped(e,
	 * 11665, 11676)) { accuracyMultiplier *= 0.15; } double cumulativeAtt = attLvl * attMult + style; return (14 + cumulativeAtt
	 ...
	 */
	/*
	 * public static double getMeleeMaximum(Player e, int attackStyle, int weaponId) { int strLvl =
	 * e.getSkills().getLevel(Skills.STRENGTH); int strBonus =
	 * e.getCombatDefinitions().getBonuses()[CombatDefinitions.STRENGTH_BONUS]; if (weaponId == -2) { strBonus += 82; } double
	 * strMult = e.getPrayer().getStrengthMultiplier(); double xpStyle = CombatDefinitions.getXpStyle(weaponId, attackStyle);
	 * double style = xpStyle == Skills.STRENGTH ? 3 : xpStyle == CombatDefinitions.SHARED ? 1 : 0; int dhp = 0; double dharokMod
	 * = 1.0; double hitMultiplier = 1.0; if (fullDharokEquipped(e)) { dhp = e.getMaxHitpoints() - e.getHitpoints(); dharokMod =
	 * (dhp * 0.001) + 1; } if (fullVoidEquipped(e, 11665, 11676)) { hitMultiplier *= 1.1; } double cumulativeStr = (strLvl *
	 * strMult + style) * dharokMod; return (int) ((14 + cumulativeStr + (strBonus / 8) + ((cumulativeStr * strBonus) / 64)) *
	 * hitMultiplier); }
	 * 
	 ...
	 */
	/*
	 * public static double getMeleeDefence(Entity e, int attackStyle, int weaponId) { boolean player = e instanceof Player; int
	 * style = player ? ((Player) e).getCombatDefinitions().getAttackStyle() : 0; style = style == 2 ? 1 : style == 3 ? 3 : 0;
	 * int defLvl = player ? ((Player) e).getSkills().getLevel(Skills.DEFENCE) : (int) (((NPC) e).getCombatLevel() * 0.6); int
	 * defBonus = player ? ((Player) e).getCombatDefinitions().getBonuses()[ CombatDefinitions.getMeleeDefenceBonus
	 * (CombatDefinitions.getMeleeBonusStyle(weaponId, attackStyle))] : 0; if (!player) { defBonus = ((NPC) e).getBonuses() !=
	 * null ? ((NPC) e).getBonuses()[ CombatDefinitions.getMeleeDefenceBonus(CombatDefinitions. getMeleeBonusStyle(weaponId,
	 * attackStyle))] : 0; } double defMult = 1.0 * (player ? ((Player) e).getPrayer().getDefenceMultiplier() : 1.0); double
	 * cumulativeDef = defLvl * defMult + style; return 14 + cumulativeDef + (defBonus / 8) + ((cumulativeDef * (defBonus)) /
	 * 64); }
	 * 
	 ...
	 */
	/*
	 * public static double getRangeAccuracy(Player e, int attackStyle) { int style = attackStyle == 0 ? 3 : attackStyle == 2 ? 1
	 * : 0; int attLvl = e.getSkills().getLevel(Skills.RANGE); int attBonus = e.getCombatDefinitions().getBonuses()[4]; double
	 * attMult = 1.0 * e.getPrayer().getRangeMultiplier() * e.getAuraManager().getRangeAccurayMultiplier(); double
	 * accuracyMultiplier = 1.05; if (fullVoidEquipped(e, 11664, 11675)) { accuracyMultiplier += 0.10; } double cumulativeAtt =
	 * attLvl * attMult + style; return (14 + cumulativeAtt + (attBonus / 8) + ((cumulativeAtt * attBonus) / 64)) *
	 * accuracyMultiplier; }
	 * 
	 ...
	 */
	/*
	 * public static double getRangeMaximum(Player e, int attackStyle) { int style = attackStyle == 0 ? 3 : attackStyle == 2 ? 1
	 * : 0; int strLvl = e.getSkills().getLevel(Skills.RANGE); int strBonus = e.getCombatDefinitions
	 * ().getBonuses()[CombatDefinitions.RANGED_STR_BONUS]; double strMult = 1.0 * e.getPrayer().getRangeMultiplier(); double
	 * hitMultiplier = 1.0; if (fullVoidEquipped(e, 11664, 11675)) { hitMultiplier += 0.1; } double cumulativeStr = strLvl *
	 * strMult + style; return (14 + cumulativeStr + (strBonus / 8) + ((cumulativeStr * strBonus) / 64)) * hitMultiplier; }
	 * 
	 ...
	 */
	/*
	 * public static double getRangeDefence(Entity e) { boolean player = e instanceof Player; int style = player ? ((Player)
	 * e).getCombatDefinitions().getAttackStyle() : 0; style = style == 2 ? 1 : style == 3 ? 3 : 0; int defLvl = player ?
	 * ((Player) e).getSkills().getLevel(Skills.DEFENCE) : (int) (((NPC) e).getCombatLevel() * 0.6); int defBonus = player ?
	 * ((Player) e).getCombatDefinitions().getBonuses()[CombatDefinitions.RANGE_DEF] : 0; if (!player) { defBonus = ((NPC)
	 * e).getBonuses() != null ? ((NPC) e).getBonuses()[9] : 0; } double defMult = 1.0 * (player ? ((Player)
	 * e).getPrayer().getDefenceMultiplier() : 1.0); double cumulativeDef = defLvl * defMult + style; return 14 + cumulativeDef +
	...
	/*
	 * public static final int getMaxHit(Player player, int weaponId, int attackStyle, boolean ranging, boolean usingSpec, double
	 * specMultiplier) { if (ranging) { return (int) (getRangeMaximum(player, attackStyle) * specMultiplier); } return (int)
	 * (getMeleeMaximum(player, weaponId, attackStyle) * specMultiplier); }
	 */
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("mindspike") || weaponName.contains("staff") || weaponName.contains("wand") || weaponName.contentEquals("obliteration"))
					return 419;
				if (weaponName.contains("dart"))
					return 6600;
				if (weaponName.contains("knife"))
					return 9055;
				if (weaponName.contains("scimitar") || weaponName.contains("sabre")) {
					switch (attackStyle) {
					case 2:
						return 412;
					default:
	...
	public static final int isRanging(Player player) {
		...
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart") || name.contains("javelin") || name.contains("thrownaxe") || name.contains("throwing axe") || name.contains("crystal bow") || name.equalsIgnoreCase("zaryte bow") || name.contains("chinchompa") || name.equals("Decimation") || name.contains("Bolas") || weaponId == 21364)
				return 2;
		}
		int ammoId = player.getEquipment().getAmmoId();
		switch (weaponId) {
