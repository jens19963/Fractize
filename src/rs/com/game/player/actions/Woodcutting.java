...
public final class Woodcutting extends Action {
	...
	public static HatchetDefinitions getHatchet(Player player) {
		...
		for (HatchetDefinitions def : HatchetDefinitions.values()) {
			if (player.getInventory().containsOneItem(def.itemId) || player.getEquipment().getWeaponId() == def.itemId) {
				hatchet = def;
	...
	public int processWithDelay(Player player) {
		...
		if (definitions.stumpId != -1 && Utils.getRandom(definitions.getRandomLifeProbability()) == 0) {
