...
public class JewllerySmithing {
	...
	public static void openInterface(Player player) {
		...
		for (int primaryIndex = 0; primaryIndex < MOLDS.length; primaryIndex++) {
			if (player.getInventory().containsOneItem(MOLDS[primaryIndex])) {
				...
				for (int secondaryIndex = 0; secondaryIndex < ITEMS[primaryIndex].length; secondaryIndex++) {
					player.getPackets().sendItems((299 + (primaryIndex * 14) + secondaryIndex), new Item[] { new Item(!player.getInventory().containsOneItem(GEMS[secondaryIndex]) ? NULL_ITEMS[primaryIndex] : ITEMS[primaryIndex][secondaryIndex]) });
				}
			} else
