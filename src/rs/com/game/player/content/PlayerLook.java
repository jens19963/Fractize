...
//		player.getPackets().sendIComponentSettings(900, 66, 0, 130, 0);
	...
	public static void handleThessaliasMakeOverButtons(Player player, int buttonId, int slotId) {
		...
		} else if (buttonId == 78 || buttonId == 79) {// color
			Integer stage = (Integer) player.getTemporaryAttributtes().get("ThessaliasMakeOver");
			player.getSocialManager().sendGameMessage("buttonId: "+buttonId+",slotId: "+ slotId +"");
			if (stage == null || stage == 0 || stage == 1)
				player.getAppearence().setTopColor(ClientScriptMap.getMap(2349).getIntValue(slotId - 16));
			else if (stage == 3)
				player.getAppearence().setLegsColor(ClientScriptMap.getMap(2349).getIntValue(slotId + 16));
		}
		player.getSocialManager().sendGameMessage("ButtonID: "+ buttonId +".");
		player.getAppearence().generateAppearenceData();
	}
	...
	public static void openThessaliasMakeOver(final Player player) {
		...
		// unlock colors
		player.getPackets().sendIComponentSettings(729, 78, 0, ClientScriptMap.getMap(2348).getSize() * 2, 0);
		player.getPackets().sendIComponentSettings(729, 79, 0, ClientScriptMap.getMap(2349).getSize() * 2, 0);
