...
public final class Commands {
	...
	public static boolean processCommand(Player player, String command, boolean console, boolean clientCommand) {
		...
		if ((player.isSupporter() || player.getRights() >= 1) && processSupportCommands(player, cmd, console, clientCommand))
			return true;
		if ((player.isSupporter() || player.getRights() >= 1) && processPunishmentCommand(player, cmd, console, clientCommand))
			return true;
	...
	public static boolean processAdminCommand(final Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "istest":
				...
				return true;
			case "st":
				player.getSlayerManager().setCurrentMaster(SlayerMaster.KURADAL);
			...
			case "removecontroler":
				...
				player.getInterfaceManager().sendInterfaces(true);
				return true;
	...
	public static void sendYell(Player player, String message, boolean staffYell) {
		...
		if (player.getRights() < 2) {
			...
			else if (player.isLegendaryDonator() && player.getRights() == 0)
				World.sendIgnoreableWorldMessage(player, "[<col=02ab2f>Legendary Donator</col>] <img=9>" + player.getDisplayName() + ": <col=02ab2f>" + message + "</col>", false);
			else
	...
	public static boolean processNormalCommand(final Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "lockxp":
				...
				player.getSocialManager().sendGameMessage("You have " +(player.isXpLocked() ? "LOCKED" : "UNLOCKED") + " your xp.");
				return true;
			case "checkvote":
			case "claim":
			case "claimvote":
