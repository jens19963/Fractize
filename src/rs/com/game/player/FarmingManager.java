	...
	public void handleFarming(SpotInfo info, Item item, int optionId) {
		...
		if (!spot.isCleared()) {
			...
			if (item != null) {
				...
				else if (WaterFilling.isFilling(player, item.getId(), true) && player.getInventory().containsOneItem(5325))
					return;
				else
		...
		player.getActionManager().setAction(new Action() {
			...
			private boolean checkAll(Player player) {
				for (HatchetDefinitions def : HatchetDefinitions.values()) {
					if (player.getInventory().containsOneItem(def.getItemId()) || player.getEquipment().getWeaponId() == def.getItemId()) {
						hatchet = def;
