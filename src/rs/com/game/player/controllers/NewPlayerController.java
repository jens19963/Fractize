...
public class NewPlayerController extends Controller {
	...
	private Dialogue getStartDialogue() {
		return new Dialogue() {
			@Override
			public void start() {
				sendEntityDialogue(Dialogue.IS_NPC, target != null ? target.getName() : "", target != null ? target.getId() : 0, 9760, "Welcome to CorruptionX!", "To start playing, you must find me using the minimap arrow and then talk to me.", "Goodluck!");
			}
	...
	private Dialogue getEndDialogue() {
		return new Dialogue() {
			@Override
			public void start() {
				sendEntityDialogue(Dialogue.IS_NPC, target != null ? target.getName() : "", target != null ? target.getId() : 0, 9760, "Good job! You found me!", "Now you can start playing and doing whatever you want.", "If you need any help or services, talk to me and I will always help you out.");
			}
	...
	public void forceClose() {
		...
		if (Settings.ECONOMY || Settings.ECONOMY_TEST) {
			String ip = player.getSession() != null ? player.getIP() : null;
